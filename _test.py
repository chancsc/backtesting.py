# to run: python _test.py
# noted: hardcoded for GOOG
...
###from backtesting.test import EURUSD, stockTicker, SMA
from backtesting.test import stockTicker, SMA

SHORT_DATA = stockTicker.iloc[:20]  # Short data for fast tests with no indicator lag
...
class TestBacktest(TestCase):
    ...
    def test_run_invalid_param(self):
        bt = Backtest(stockTicker, SmaCross)
    ...
    def test_data_missing_columns(self):
        df = stockTicker.copy(deep=False)
        del df['Open']
    ...
    def test_data_invalid(self):
        with self.assertRaises(TypeError):
            Backtest(stockTicker.index, SmaCross).run()
        with self.assertRaises(ValueError):
            Backtest(stockTicker.iloc[:0], SmaCross).run()
    ...
    def test_assertions(self):
        ...
        bt = Backtest(stockTicker, Assertive)
        with self.assertWarns(UserWarning):
            stats = bt.run()
        self.assertEqual(stats['# Trades'], 145)

    def test_broker_params(self):
        bt = Backtest(stockTicker.iloc[:100], SmaCross,
    ...
    def test_compute_stats(self):
        stats = Backtest(stockTicker, SmaCross).run()
        expected = pd.Series({
                # NOTE: These values are also used on the website!
                '# Trades': 66,
    ...
    def test_compute_stats_bordercase(self):
        ...
        for strategy in (SmaCross,
            ...
            with self.subTest(strategy=strategy.__name__):
                stats = Backtest(stockTicker.iloc[:100], strategy).run()
    ...
    def test_trade_enter_hit_sl_on_same_day(self):
        ...
        self.assertEqual(Backtest(stockTicker, S).run()._trades.iloc[0].ExitPrice, 720)
        ...
        with self.assertWarns(UserWarning):
            self.assertEqual(Backtest(stockTicker, S).run()._trades.iloc[0].ExitPrice, 705.58)
    ...
    def test_stop_price_between_sl_tp(self):
        ...
        with self.assertWarns(UserWarning):
            self.assertEqual(Backtest(stockTicker, S).run()._trades.iloc[0].EntryPrice, 104)
    ...
    def test_position_close_portion(self):
        ...
        bt = Backtest(stockTicker, SmaCross, commission=.002)
        bt.run()
...
class TestPlot(TestCase):
    ...
    def test_range_axis(self):
        df = stockTicker.iloc[:100].reset_index(drop=True)
    ...
    def test_preview(self):
        ...
        bt = Backtest(stockTicker, Strategy)
        bt.run()
        with _tempfile() as f:
    ...
    def test_wellknown(self):
        ...
        bt = Backtest(stockTicker, S, margin=.1)
        stats = bt.run()
...
class TestLib(TestCase):
    ...
    def test_random_ohlc_data(self):
        generator = random_ohlc_data(stockTicker, frac=1)
        new_data = next(generator)
        self.assertEqual(list(new_data.index), list(stockTicker.index))
        self.assertEqual(new_data.shape, stockTicker.shape)
        self.assertEqual(list(new_data.columns), list(stockTicker.columns))

    def test_compute_stats(self):
        stats = Backtest(stockTicker, SmaCross).run()
        only_long_trades = stats._trades[stats._trades.Size > 0]
        long_stats = compute_stats(stats=stats, trades=only_long_trades,
                                   data=stockTicker, risk_free_rate=.02)
    ...
    def test_SignalStrategy(self):
        ...
        stats = Backtest(stockTicker, S).run()
...
class TestUtil(TestCase):
    ...
    def test_pandas_accessors(self):
        ...
        Backtest(stockTicker.iloc[:20], S).run()
